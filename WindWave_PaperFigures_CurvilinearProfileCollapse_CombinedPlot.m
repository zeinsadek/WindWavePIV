...
clc; close all
ax = figure('color', 'white', 'units', 'centimeters', 'position', [10,10,13,9.5]);
tile = tiledlayout(2,2,'TileSpacing', 'loose', 'Padding', 'compact');

all_profiles_tile = nexttile([1,2]);
...
for s = 1:length(wind_speeds)
    ...
    x_label_positions = [0.06, 0.28, 0.52];
    y_label_positions = [0.86, 0.86, 0.86];
    annotation(ax, 'textbox', [x_label_positions(s), y_label_positions(s), 0.3, 0.04], ...
...
set(all_profiles_tile, 'TickLabelInterpreter', 'latex', 'FontSize', tickFontSize)
ylabel('$\zeta \mathbin{/} \delta$', 'interpreter', 'latex', 'fontsize', labelFontSize)
xlabel('$\langle u_{\xi} \rangle$ [m/s]', 'interpreter', 'latex', 'fontsize', labelFontSize)
xticks(1:6)
yticks(0:0.5:2)
xlim([1, 5.6])
xticks(1:1:6)
...
leg = legend('interpreter', 'latex', 'fontsize', legendFontSize, 'box', 'off');
...
addPanelLabelsFixed(ax, [all_profiles_tile, h], {'a', 'b', 'c'}, 'Fontsize', 10, 'OffsetPts', [-25, -8])
