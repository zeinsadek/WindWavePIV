...
%% Shear stress contours: curvilinear
...
%     wind_speed = wind_speeds{s};
%     u_inf = freestreams.(wind_speed);
%     no_wave_max = no_wave_max_stress.(wind_speed) / (u_inf^2);
% 
%     for w = 1:length(waves)
%         wave = waves{w};
% 
...
%% Diagnosing specific wind speed, and phase
...
%     y_profile = y_profile(1:end - buffer);
%     profile = profile(1:end - buffer);
%     plot(profile, y_profile, 'color', wave_colors{w}, 'linewidth', 2)
% end
% hold off
% 
% 
...
%% PLOT: All phases combined with single fit
...
plot(wave_age_fit, u_star_fit_all, 'k-', 'linewidth', 2)
...
patch([wave_age_fit(:); flipud(wave_age_fit(:))], ...
...
text(0.2, 0.45, sprintf('$u^*_{\\zeta} = %.3f \\left( c \\mathbin{/} U_\\infty \\right)^{%.2f}$', A_all, B_all), ...
     'interpreter', 'latex', 'fontsize', 10)
text(0.2, 0.40, sprintf('$R^2 = %.2f$', R2_all), ...
     'interpreter', 'latex', 'fontsize', 10)
...
for phase = 1:4    
    for s = 1:length(wind_speeds)
        ...
        for w = 1:length(waves)
            ...
            % Plot
            scatter(wave_age, u_star, 2 * sz, phase_markers{phase}, 'filled', ...
...
% Add fit equation and R^2 to plot
text(0.18, 0.48, sprintf('$u^*_{\\zeta} = %.3f \\left( c \\mathbin{/} U_\\infty \\right)^{%.2f}$', A_all, B_all), ...
...
ylabel(h(1), '$u_{\zeta}^*$ [m/s]', 'interpreter', 'latex', 'fontsize', labelFontSize)
