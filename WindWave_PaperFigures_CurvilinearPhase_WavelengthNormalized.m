...
% phase_labels = {'Peak: $\varphi = 0$', 'Ascending: $\varphi = \lambda / 4$', ...
%                 'Trough: $\varphi = \lambda / 2$', 'Descending: $\varphi = 3 \lambda / 4$'};
...
for phase = 1:4
    ...
    xticks((0 / wave_length):0.25:(250 / wave_length))
    
    % title(sprintf('Phase %1.0f', phase))
    title(phase_labels{phase}, 'interpreter', 'latex', 'FontSize', labelFontSize)

    if phase == 1
        ylabel('$y / \lambda$', 'interpreter', 'latex', 'FontSize', labelFontSize)
    end
...
xlabel(t, '$x / \lambda$', 'interpreter', 'latex', 'FontSize', labelFontSize);
linkaxes(h,'xy')

% Save figure
% fig_folder = '/Users/zeinsadek/Desktop/Experiments/Offshore/wind_wave_PIV/paper_figures/new/pdf_test3/Phase';
...
hold off
xlabel('$\zeta / \lambda$', 'interpreter', 'latex', 'FontSize', labelFontSize)
...
annotation(totalFigure, 'textbox', [0.23, 0.48, 0.5, 0.04], ...
    'String', '$x / \lambda$', ...
    'Interpreter', 'latex', ...
