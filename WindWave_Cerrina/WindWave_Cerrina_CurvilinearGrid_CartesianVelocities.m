...
for phase = 1:4
    ...
    % for i = 1
        ...
        % Save
        % Instantaneous crop 
        Cartesian_instantaneousInterpolated(phase).instantaneous_crop.u(:,:,i) = u_inst_interp;
        Cartesian_instantaneousInterpolated(phase).instantaneous_crop.v(:,:,i) = v_inst_interp;

        % Max wave profile crop
        Cartesian_instantaneousInterpolated(phase).maximum_crop.u(:,:,i) = u_inst_interp_max_crop;
        Cartesian_instantaneousInterpolated(phase).maximum_crop.v(:,:,i) = v_inst_interp_max_crop; 

        % Save waves
        Cartesian_instantaneousInterpolated(phase).waves(i,:) = wave_inst;
    ...
    Cartesian_instantaneousInterpolated(phase).num_images = num_images;
...
for phase = 1:4
    ...
    mean_u  = mean(Cartesian_instantaneousInterpolated(phase).(crop_method).u, 3);
    mean_v  = mean(Cartesian_instantaneousInterpolated(phase).(crop_method).v, 3);
    u_flucs = Cartesian_instantaneousInterpolated(phase).(crop_method).u - mean_u;
    v_flucs = Cartesian_instantaneousInterpolated(phase).(crop_method).v - mean_v;
    ...
    % Save
    Cartesian_meansFromInterpolatedInstantaneous(phase).u  = mean_u;
    Cartesian_meansFromInterpolatedInstantaneous(phase).v  = mean_v;
    Cartesian_meansFromInterpolatedInstantaneous(phase).uu = uu;
    Cartesian_meansFromInterpolatedInstantaneous(phase).vv = vv;
    Cartesian_meansFromInterpolatedInstantaneous(phase).uv = uv;
end
...
% Cartesian velocities, interpolated onto a curvilinear grid
output.Cartesian_instantaneous = Cartesian_instantaneousInterpolated;
output.Cartesian_means = Cartesian_interpolatedMeans;
...
%%% Save to matfile
save_folder = '/Users/zeinsadek/Desktop/Experiments/Offshore/wind_wave_PIV/curvilinear_cerrina';
caze_name   = [caze, '_CartesianInstantaneous_curvilinearProjected.mat'];
