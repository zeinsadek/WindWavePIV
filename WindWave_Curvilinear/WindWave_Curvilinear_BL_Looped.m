...
clear alpha caze data H linestyles lw parameter phase reference s scale cersion w wave wind_speed version ans




%% Plot average of phases of specific parameter for all waves and wind speeds
...
for s = 1:length(wind_speeds)
    ...
    for w = 1:length(waves)
        ...
        for phase = 1:4
            data = integral.(caze)(phase).(version).(parameter);
            value = data(round(length(data) / 2));

            tmp_mean(phase) = value;

            scatter(steepness, value, 100, 'filled', wind_speed_shapes{s},'markerfacecolor', wave_colors{w});
        end

        scatter(steepness, mean(tmp_mean, 'all', 'omitnan'), 200, 'filled', wind_speed_shapes{s},'markerfacecolor', 'black')
        clear tmp_mean
...
%% Trying to generalize by plotting parameters against wave characteristics
...
%     wind_speed = wind_speeds{s};
% 
%     geometric_steepnesses = nan(1,4);
%     tmp_avg_of_phase = nan(1,4);
% 
%     for w = 1:length(waves)
%         wave = waves{w};
% 
...
%         waveheight = 2 * integral.(caze)(1).amplitude;
%         geometric_steepness = waveheight / wavelength;
%         geometric_steepnesses(w) = geometric_steepness;
% 
%         tmp = nan(1,4);
% 
%         % Iterate through waves
...
%             wave_informed_freestream = (displacement ./ thickness);
% 
%             idx = round(length(displacement)/2);
% 
%             scatter(geometric_steepness, wave_informed_freestream(idx), sz, markers{s}, 'filled', ...
%                     'MarkerEdgeColor', 'none', 'MarkerFaceColor', wave_colors{w}, 'MarkerFaceAlpha', 0.5);
% 
% 
%             % Compute average-of-phases
%             tmp(i) = integral.(caze)(p).filtered.displacement(idx) / integral.(caze)(p).filtered.thickness(idx);
% 
%        end
%        tmp_avg_of_phase(w) = mean(tmp, 'all', 'omitnan');
%        scatter(geometric_steepness, mean(tmp, 'all', 'omitnan'), sz, 'filled', ...
%                     'MarkerEdgeColor', 'none', 'MarkerFaceColor', 'black');
%     end
% 
%     [geometric_steepnesses, sortIdx] = sort(geometric_steepnesses, 'descend');
%     tmp_avg_of_phase = tmp_avg_of_phase(sortIdx);
%     plot(geometric_steepnesses, tmp_avg_of_phase, 'color', 'black');
%     % P.Color(4) = 0.5;
% 
% end
% hold off
