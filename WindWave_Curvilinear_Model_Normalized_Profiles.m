...
for s = 1:length(wind_speeds)
    ...
    for w = 1:length(waves)
        ...
        for phase = 1:4
            ...
            %%% Model the \delta^* / \delta parameter
            modeled_parameter = -0.0 * cos(2 * pi * (phase - 1)/4) + 0.17;
            ...
            modeled_normalized_u = (u_inf - u_profile) / (u_inf * modeled_parameter);
            normalized_y = y_profile / thickness(idx);

            P = plot(corrected_normalized_u, normalized_y, ...
                     'color', 'red', 'linestyle', line_styles{s}, 'linewidth', lw, 'displayname', caze);
            % P.Color(4) = alpha;

            P = plot(modeled_normalized_u, normalized_y, ...
                     'color', 'blue', 'linestyle', line_styles{s}, 'linewidth', lw, 'displayname', caze);
            % P.Color(4) = alpha;
