...
plot((hysteresis(2).x) + 0.25, hysteresis(2).cf, ...
     'linewidth', lw, 'color', 'red', 'displayname', 'Phase 2 Data')

plot((hysteresis(4).x + 0.5) + 0.25, hysteresis(4).cf, ...
     'linewidth', lw, 'color', 'blue', 'linestyle', '-', 'Displayname', 'Phase 4 Raw')

plot((hysteresis(4).x + 0.5) + 0.25, phase_4_cf_shifted, ...
     'linewidth', lw, 'color', 'green', 'Displayname', 'Phase 4 Shifted')
...
hold off
xline(0.5, 'color', 'black', 'linestyle', '-', 'linewidth', lw, 'HandleVisibility', 'off')
ylabel('$C_f$', 'interpreter', 'latex', 'fontsize', labelFontSize)
ylim([Cf_LL, Cf_UL])
yticks(Cf_ticks)
xlim([0, 1])
...
reference = plot(0.5 + (integral.(caze)(1).wave.x  - mean(integral.(caze)(1).wave.x, 'all', 'omitnan')) / integral.(caze)(1).wavelength, (Re_theta_reference_scale * integral.(caze)(1).wave.wave_profile) + Re_theta_reference_offset, ...
                           'color', 'black', 'linewidth', lw, 'linestyle', '--', 'handlevisibility', 'on', 'Displayname', 'Reference Wave');
reference.Color(4) = 0.5;
...
leg = legend('Orientation', 'vertical', 'box', 'off', 'fontsize', legendFontSize);
...
for w = 1:length(waves)
    ...
    for s = 2
        ...
        % Mark where peak is
        scatter(phase_4_Re_theta_shifted(1), phase_4_cf_shifted(1), 4*sz, 'pentagram', 'filled', 'MarkerFaceColor', 'black', ...
                'MarkerFaceAlpha', wind_speed_alphas(s), 'MarkerEdgeAlpha', wind_speed_alphas(s))
        ...
        title(sprintf('$\\lambda_{%s}$', wavelength_names.(wave)), 'interpreter', 'latex', 'fontsize', titleFontSize)
...
% Save figure
% figure_name = strcat('Hysteresis_', wind_speeds{s}, '.png');
...
xlabel('$H /\lambda$', 'interpreter', 'latex', 'fontsize', labelFontSize)
...
% Save figure
% figure_name = 'Hysteresis_Ranges.png';
% exportgraphics(ax, fullfile(figure_folder, figure_name), 'resolution', 300)
% close all
...
% Save figure
% pause(3)
% figure_name = 'Hysteresis_Closures.png';
% exportgraphics(ax, fullfile(figure_folder, figure_name), 'resolution', 300)
% close all
