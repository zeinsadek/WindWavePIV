%%% Wind + Wave curvilinear u* check with momentum integral equation
...
for s = 1:length(wind_speeds)
    ...
    for w = 1:length(waves)
        ...
        x_w_units = integral.(caze)(phase).x; 

        %%% Compute 2 * \frac{d \theta}{dx}
        cf = 2 * (gradient(data) ./ gradient(x_w_units));

        H(w) = plot((integral.(caze)(phase).x - mean(integral.(caze)(phase).wave.x, 'all', 'omitnan')) / integral.(caze)(1).wavelength, cf, ...
                    'color', wave_colors{w}, 'linewidth', lw, 'linestyle', linestyles{s}, ...
                    'displayname', label);

        % Plot phase for reference
        if wave == 'D' && s == 1
            reference = plot((integral.(caze)(phase).wave.x  - mean(integral.(caze)(phase).wave.x, 'all', 'omitnan')) / integral.(caze)(1).wavelength, (scale * integral.(caze)(phase).wave.wave_profile), ...
