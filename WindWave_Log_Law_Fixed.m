...
LL_file       = strcat(results_path, 'log_law_fixed', '/', experiment_name, '_LL_Fixed.mat');
...
%%% Mean Momentum Flux
% Compute integrad
[du2dx, du2dy] = gradient(means.ensemble.u.^2, dx);
...
% Compute integral
int_du2dx_dy = cumtrapz(flipud(y_mask), flipud(du2dx_profile));
int_du2dx_dy = -1 * flipud(int_du2dx_dy);
...
output.ensemble.IBL.mean_mom_mask = du2dx_profile_mask;
output.ensemble.IBL.mean_mom      = term_3;

% figure()
% hold on
% plot(int_du2dx_dy, y_mask * 1E3, 'DisplayName', 'A')
% plot(umean_vmean, y, 'DisplayName', 'B')
% plot(term_3, y(du2dx_mask), 'red', 'DisplayName', 'A + B')
% hold off
% legend()
...
% Compute integrad
[duudx, duudy] = gradient(means.ensemble.uu, dx);
...
% Compute integrals
int_duudx_dy = cumtrapz(flipud(y_mask), flipud(duudx_profile));
int_duudx_dy = -1 * flipud(int_duudx_dy);
...
%%% Pressure Correction
% Compute integrad
[dvvdx, dvvdy] = gradient(means.ensemble.vv, dx);
...
% Compute integrals
int_dvvdx_dy = cumtrapz(flipud(y_mask), flipud(dvvdx_profile));
int_dvvdx_dy = flipud(int_dvvdx_dy);
...
%%% Pressure Gradient
u_inf = BL_data.u_inf;
u_masked = means.ensemble.u;
u_masked(Y < BL_data.ensemble.thickness) = nan;
[du_infdx, du_infdy] = gradient(u_masked, dx);
du_infdx_avg = mean(du_infdx(:, profile_idx - profile_buffer:profile_idx + profile_buffer), 'all', 'omitnan');
...
if wave_type ~= '0'
    ...
    for i = 1:length(phase_offset)
        ...
        phase_u_profile = means.phase(i).u(:, profile_idx);

        %%% CORRECTION ADDED:
        % Filter profiles in the same way we are doing for curvilinear
        % Apply same filtering as curvilinear code
        phase_u_profile = StatisticalGradientFilter(phase_u_profile, stat_grad_tol, max_nan_island);
        phase_u_profile(find(~isnan(phase_u_profile), 1, 'first')) = nan;

        % Get reference_profile
        reference_wave_profile = means.phase(i).reference_wave;
        local_wave_height = reference_wave_profile(profile_idx);
        ...
        % y needs to be zeroed to the wave surface for phase averages

        phase_u_plus(:,i) = phase_u_profile / u_star;
        % phase_y_plus(:,i) = (y * (10^-3) * u_star) / nu;
        phase_y_plus(:,i) = ((y - local_wave_height) * (10^-3) * u_star) / nu;
